<div class="uk-grid" data-uk-grid-margin>
    <div class="uk-width-1-1">
    	...
    	<form role="form" method="POST" action="{{ url('invoice_debit_credit/update') }}" id="data-form-edit" data-redirect-on-success="{{ url('invoices/rows/'.$invoicedebit_credit->invoice_id) }}">
    	    {{ csrf_field() }}
    	    <input type="hidden" name="id" value="{{ $invoicedebit_credit->id }}">
    		<div class="uk-width-medium-1-1 uk-row-first">
    		    ...
    		    <div class="md-input-wrapper md-input-filled md-input-select">
                    ...
                    <select name="signs" id="signs" data-md-selectize>
                        <option value="">{{ __('projects.signs') }}...</option>
                            ...
                            <option value="+" {{ ($invoicedebit_credit->signs=='+')? 'selected':''}}>+</option>
                       <option value="-"  {{ ($invoicedebit_credit->signs=='-')? 'selected':'' }}>-</option>
                    </select>
                </div>
    			...
    			<div class="md-input-wrapper md-input-filled">
                	...
                	<input type="text" class="md-input" name="detail" id="detail" value="{{ $invoicedebit_credit->detail }}" required><span class="md-input-bar"></span>
                </div>
                <div class="parsley-errors-list filled"><span class="parsley-required detail-error"></span></div>

                <div class="md-input-wrapper md-input-filled">
                	<label>{{ __('invoices.cost') }}</label>
                	<input type="text" class="md-input" name="cost" id="debit_creditcost" value="{{ $invoicedebit_credit->cost }}" required><span class="md-input-bar"></span>
                </div>
				...
				<div class="md-input-wrapper md-input-filled">
					...
					<input type="text" class="md-input" name="amount" id="debit_creditamount" value="{{ $invoicedebit_credit->amount }}" required><span class="md-input-bar"></span>
				</div>
                ...
                <!--<div class="md-input-wrapper md-input-filled md-input-select">
                	...
                	<select name="currency_id" id="currency_id" data-md-selectize>
                	    ...
                	    @foreach ($currencies as $currency)
                	        <option value="{{ $currency->id }}" {{ ($currency->id == $invoicedebit_credit->currency_id) ? 'selected' : '' }}>{{ $currency->name }}</option>
                	    @endforeach
                	</select>
                </div>
