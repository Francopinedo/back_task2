...
@section('scripts')
 ...
 var whatif_id = '{{ $whatif_id }}';
    ...
    <script src="{{ asset('js/whatif_tasks.js') }}"></script>

    <script>
        $(document).ready(function () {
            tasks.init({{ session('project_id') }},'{{$whatif_id}}', '{{ $project->start}}', '{{ isset($project->hours_by_day)&&!empty($project->hours_by_day)?$project->hours_by_day:8}}');
...
@section('section_title', __('whatif_tasks.whatif_tasks'))
