<div class="uk-grid" data-uk-grid-margin>
    <div class="uk-width-1-1">
    	...
    	<form role="form" method="POST" action="{{ url('whatif_task_resources') }}" id="data-form-ajax_create" data-redirect-on-success="{{ url('whatif_tasks/'.$task->id.'/rows') }}">
    	    {{ csrf_field() }}
    	    <input type="hidden" name="whatif_task_id" value="{{ $task->id }}">
    	    <input type="hidden" name="company_id" id="company_id" value="{{ $company->id }}">
            <input type="hidden" name="whatif_task_estimated_hours" id="company_id" value="{{ $task->estimated_hours }}">
    		<div class="uk-width-medium-1-1 uk-row-first">


                <label>{{ __('whatif_tasks.role') }}</label>
                <div class="md-input-wrapper md-input-select">
                	<select name="project_role_id" id="project_role_id" >
                	    <option value="">{{ __('whatif_tasks.role') }}...</option>
                ...
                <label>{{ __('whatif_tasks.user') }}</label>
                <div class="md-input-wrapper md-input-select">
                    <select name="user_id" id="user_id" >
                        <option value="">{{ __('whatif_tasks.user') }}...</option>
                ...
                <div class="md-input-wrapper">
                    <label>{{ __('whatif_tasks.rate') }}</label>
                ...
                <div class="md-input-wrapper">
                    <label>{{ __('whatif_tasks.hours') }}</label>
