 ...
 var extra_buttons = [{
            ...
            action: function (e, dt, node, config) {
                ...
                var $switcher_ajax_create = $('#ajax_create_div'),
                    ...
                    $ajax_create_url = 'company_roles/import';
            ...
            var actions = [
                {
                    pre: '<a title={{__('general.edit')}} href="/company_roles/',
                    post: '/edit" class="table-actions edit-btn"><i class="fa fa-pencil" aria-hidden="true"></i></a>'
                },
                {
                    pre: '<a title={{__('general.delete')}} href="/company_roles/',
            ...
            <div class="uk-grid" data-uk-grid-margin>
                <div class="uk-width-1-1">
                    ...
                    <table id="company_roles-table" class="uk-table" cellspacing="0" width="100%">
                        <thead>
                        <tr>
                            <th title="{{__('company_roles_tooltip.id')}}">{{ __('company_roles.id') }}</th>
                            <th title="{{__('company_roles_tooltip.title')}}">{{ __('company_roles.title') }}</th>
                        ...
                        <div class="uk-width-medium-1-3">
                           <!-- <a class="md-btn md-btn-primary md-btn-wave-light waves-effect waves-button waves-light" href="#" id="reload">{{ __('holidays.reload') }}</a>-->
                            <a class="md-btn md-btn-primary md-btn-wave-light waves-effect waves-button waves-light"
                               href="#" id="add-new" title="{{ __('company_roles.add_new') }}">{{ __('company_roles.add_new') }}</a>
                        </div>
                    </div>
                </div>
            </div>
