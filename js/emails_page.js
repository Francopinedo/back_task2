	...
	var emails = {
		init: function(){
            ...
            $('#reload_emails').on('click', function(e){
                console.log('reload');
                e.preventDefault();
                if (window.confirm('Templates with the same name will be overwritten. Are you sure you want to overwrite them?')) {
            	...
            	UIkit.modal.prompt(
            		...
            		function(val){
        				...
        				$.ajax({
        			        ...
        			        type: 'POST',
        			        dataType: 'json',
        			        data: {title: val, company_id: company_id, user_id: $('#user_id').val() },
        				    success : function ( json )
        				    {
        				    	location.reload();
