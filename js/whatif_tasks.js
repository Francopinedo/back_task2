...
var tasks = (function () {
    ...
    var tasks = {
            ...
            minDate: '',
            hours_by_day: '',
            init: function (project_id,whatif_id, minDate, hours_by_day) {
                ...
                self.loadTasks(project_id,whatif_id);
             ...
             * loadTasks
             */
            loadTasks: function (project_id,whatif_id) {
                var self = this;


                $.ajax({
                    url: API_PATH + 'whatif_task?project_id=' + project_id+'&whatif_id='+whatif_id,
                    type: 'get',
                    dataType: 'json'
                }).done(function (data) {
            ...
            addTask: function () {
                ...
                if (description != '' && start_date != '' && due_date != '') {
                    ...
                    ge.tasks[index].whatif_id=whatif_id
...
//                kendo.ui.progress(element, false);
                ...
                window.location.href = baseUrl+'/dashboard';
                   ...
                   //var self = this;
              // tasks.loadTasks(self.project_id,self.whatif_id);
             //  tasks.redraw();
                var getUrl = window.location;
                var baseUrl = getUrl .protocol + "//" + getUrl.host;
                window.location.href = baseUrl+'/dashboard';
            ...
            initEditForm: function () {
                ...
                $('#update-task-btn').click(function (e) {
                    ...
                    $.ajax({
                        url: API_PATH + 'whatif_task/'+task.id,
                        type: 'POST',
                        async: false,
                        data: $('#data-form-edit').serializeArray(), 
                        dataType: 'json',
                        success: function (json) {
                        self.loadTasks(self.project_id,self.whatif_id);
                         self.redraw();
                        },
                        error: function (json) {
                            console.log('error');
            ...
            saveOnServer: function () {
                ...
                if (ge.deletedTaskIds > 0) {
                    ...
                    $.ajax({
                        url: API_PATH + 'whatif_task/delete/all',
                        type: 'DELETE',
                        async: false,
                ...
                $.each(ge.tasks, function (index, value) {
                    ...
                    if (Math.floor(value.id) == value.id && $.isNumeric(value.id)) { // si es una tarea existente, que tiene ID, entonces la actualizo
                        ...
                        url = API_PATH + 'whatif_task/' + value.id;
                        verb = 'PATCH';
                        ...
                        console.log('nueva');
                        url = API_PATH + 'whatif_task';
                        verb = 'POST';
                        data.project_id = self.project_id;
                        data.whatif_id = self.whatif_id;
                   ...
                   if (tasksToAdd.length > 0) {
                           ...
                           var url = API_PATH + 'whatif_task/store/all';
                            $.ajax({
                                url: url,
                                type: 'POST',
                         ...
                         if (tasksToAdd.length == 0 && tasksToEdit.length > 0) {
                           ...
                           var url = API_PATH + 'whatif_task/update/all';
                            $.ajax({
                                url: url,
                                type: 'POST',
            ...
            initRedirecction: function () {
                ...
                $('.go-to-tickets').unbind().on('click', function (e) {
                    ...
                    var win = window.open("/whatif_tasks/" + $(this).data('id') + "/tickets", '_blank');
                    if (win) {
                ...
                $('.go-to-rows').unbind().on('click', function (e) {
                    ...
                    if (tasks.checkId($(this).data('id'))) {
                        e.preventDefault();
                        var win = window.open("/whatif_tasks/" + $(this).data('id') + "/rows", '_blank');
                        if (win) {
                            //Browser has allowed it to be opened
                            win.focus();
                        } else {
