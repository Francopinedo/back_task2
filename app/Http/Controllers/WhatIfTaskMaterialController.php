...
class WhatIfTaskMaterialController extends Controller
{
    ...
    public function create($whatif_task_id){
    	...
    	return response()->json([
    		'view' => view('whatif_task_material/create', [
    ...
    return response()->json($validator->errors(), 422);
    	...
    	$res = $this->apiCall('POST', 'whatif_task_materials', $data);
    ...
    public function edit($id){
    	$company = $this->getFromApi('GET', 'companies/fromUser/'.Auth::id());
    	$taskMaterial = $this->getFromApi('GET', 'whatif_task_materials/'.$id);
    	 $currencies = $this->getFromApi('GET', 'currencies');
    	 $materials = $this->getFromApi('GET', 'materials?company_id='.$company->id);

    	return response()->json([
    		'view' => view('whatif_task_material/edit', [
    ...
    return response()->json($validator->errors(), 422);
    	...
    	$res = $this->apiCall('PATCH', 'whatif_task_materials/'.$data['id'], $data);
    ...
    {
    	$taskMaterial = $this->getFromApi('GET', 'whatif_task_materials/'.$id);
    	$res = $this->apiCall('DELETE', 'whatif_task_materials/'.$id);
    	...
    	if (!isset($jsonRes))
    	{
    		return redirect('whatif_tasks/'.$taskMaterial->whatif_task_id.'/rows');
    	}
    	else
    	{
