...
class SOXAudit
{
...

	{
		...
		if($method!='login' || $method!='index' || $method!='edit'|| $method!='create'|| $method!='select' || $method!='forContracts' || $method!='rawDirectory'|| $method!='forProjectSelection')
		{
		
	$data['date_action'] = $now->toDateTimeString();
	$data['process_name'] =$method .'for Process '.$controller;
	...
	empty(Auth::id())?\App\User::find(Auth::id())->name:'';
		$data['user_comment'] ='SOX AUDIT LOG';
	$data['reason'] ='SOX AUDIT LOG  '.$method;
	$data['action'] =$method;
...
$data['role'] =isset($request->user_id)?\App\Role::find(\App\RoleUser::where('user_id',$request->user_id)->first()->role_id)->name: isset(Auth::user())? \App\Role::find(\App\RoleUser::where('user_id',Auth::user()->id)->first()->role_id)->name: 'N/A';
