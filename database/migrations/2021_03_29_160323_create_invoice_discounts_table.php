...
class CreateInvoiceDiscountsTable extends Migration {
	...
	public function up()
	{
		Schema::create('invoice_discounts', function(Blueprint $table)
		{
			$table->increments('id');
			$table->timestamps();
			$table->integer('invoice_id')->unsigned()->index('invoice_discounts_invoice_id_foreign');
			$table->integer('currency_id')->unsigned()->index('invoice_discounts_currency_id_foreign');
