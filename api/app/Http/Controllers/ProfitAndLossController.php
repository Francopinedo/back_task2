      ...
      ";
        ...
        foreach ($data as $contractResource) {
            ...
            // foreach ($period as $dt) {
                ...
                if(!empty($workingHoursFromApi->working_hours)){
                if ($workingHoursFromApi->working_hours >= 0) {
                    $totaluser = $totaluser + $workingHoursFromApi->working_hours;
                }
              }
              if(!empty($workingHoursFromApi->hours_available)){
                if ($workingHoursFromApi->hours_available >= 0) {
                    $hours_available = $hours_available + $workingHoursFromApi->hours_available;

                }
              }
              if(!empty($workingHoursFromApi->efective_capacity)){
              ...
              if(!empty($workingHoursFromApi->holidays_hours)){
              ...
              if(!empty($workingHoursFromApi->absents_hours)){

                $absents_hours = $absents_hours + $workingHoursFromApi->absents_hours;
              }
              if(!empty($workingHoursFromApi->hours_asigned)){              
                $hours_asigned = $hours_asigned + $workingHoursFromApi->hours_asigned;
              }
              if(!empty($workingHoursFromApi->replacements_hours)){              
        ...
        $total_cost = 0;
        $total_real_hours = 0;
        $invoice_currency_exchange = ExchangeRate::where('currency_id', $request->currency_id)->where('company_id', $request->company_id)->first();
