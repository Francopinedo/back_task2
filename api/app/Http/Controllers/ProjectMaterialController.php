...
use Transformers\ProjectMaterialTransformer;

/**
 * Modulo de ProjectMaterial
 *
 * @Resource("Group ProjectMaterial")
 */
class ProjectMaterialController extends Controller {
	 ...
	 * Obtener
	 *
	 * @Get("project_materials{?company_id}")
	 * @Parameters({
	 ...
	 */
  	...
  	{
  		$query = ProjectMaterial::with('project');
  		...
  		return $this->response->collection($projectMaterials, new ProjectMaterialTransformer);
  	}
      ...
      public function index_export(Request $request)
    {
        ...
        $query = DB::table('project_materials')
                    ->select(
                      'project_materials.detail',
                      'project_materials.cost',
                      'project_materials.amount',
                       'project_materials.frequency',
                      'project_materials.reimbursable',
        ...
        if ($request->has('project_id'))
      {
        $query->where('project_materials.project_id', $request->project_id);
      }
    ...
    $projectMaterials = $query->get();
        ...
        $monthly = 'monthly';
        foreach ($projectMaterials as $contractResource) {
	 ...
	 */
  	public function store(Request $request)
  	{
        ...
        $projectMaterial = ProjectMaterial::create($data);

        if ($projectMaterial)
        {
        	return $this->response->item($projectMaterial, new ProjectMaterialTransformer);
        }
        else
    	{
	 ...
	 */
  	...
  	{
  		$projectMaterial = ProjectMaterial::findOrFail($id);
	 ...
	 */
  	...
  	{
  		$projectMaterial = ProjectMaterial::find($id);
	 ...
	 * @Get("project_materials/datatables")
	 */
